...
export default function EditCollectionDrawer({ isOpen, onClose, dataset }: Props) {
  ...
  const filtered =
    tab === "selected"   ? searched.filter((d) => selectedIds.has(d.id))
    : tab === "unselected" ? searched.filter((d) => !selectedIds.has(d.id))
    : searched;
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-[480px] z-50 flex flex-col bg-background border-l border-border shadow-xl">

        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-border shrink-0">
          <div>
            <p className="text-[15px] font-semibold text-foreground">Edit collection</p>
            <p className="text-[12px] text-muted-foreground mt-0.5">
              {selectedIds.size} of {allDocuments.length} {allDocuments.length === 1 ? "file" : "files"} selected
            </p>
          </div>
        ...
        {/* Footer */}
        <div className="shrink-0 px-5 py-4 border-t border-border flex items-center justify-end gap-4">
...
function FileRow({ doc, selected, onToggle }: { doc: Document; selected: boolean; onToggle: () => void }) {
